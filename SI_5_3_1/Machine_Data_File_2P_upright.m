...
shutterDaqDevices = {'PXI1Slot5'};  % Cell array specifying the DAQ device or RIO devices for each shutter eg {'PXI1Slot3' 'PXI1Slot4'}
...
beamDaqs(1).displayNames = {'Beam1-ResScan'};                  % Optional string cell array of identifiers for each beam
...
beamDaqs(1).calOffsets = 0.0102238;                    % Array of beam calibration offset voltages for each beam calibration channel
...
motors(1).positionDeviceUnits = [1e-07 1e-07 4e-08];      % 1xN array specifying, in meters, raw units in which motor controller reports position. If unspecified, default positionDeviceUnits for stage/controller type presumed.
...
channelsInvert = [true false true false];             % Logical: Specifies if the input signal is inverted (i.e., more negative for increased light signal)
